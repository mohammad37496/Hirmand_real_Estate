  ...
  .handler(async ({ data }) => {
    ...
    await sql.query(
      ...
      on conflict (id) do update set
        ...
        rent = excluded.rent,
        previous_price = case when $27 is not null then $27 else properties.previous_price end,
        previous_deposit = case when $28 is not null then $28 else properties.previous_deposit end,
        previous_rent = case when $29 is not null then $29 else properties.previous_rent end,
        price_changed_at = case when $30::numeric is not null then current_timestamp else properties.price_changed_at end,
        price_drop_percent = case when $30::numeric is not null then $30::numeric else null end,
