...
export default defineEventHandler(async (event) => {
  ...
  const updateBudgetLead = async (leadId: string) => {
    await sql.query(
      `update leads
  ...
  try {
    if (!guardRows[0]?.acquired) {
      ...
      if (duplicate && parsed.data.source === "budget_match") {
        await updateBudgetLead(duplicate.id);
    ...
    if (existing[0]) {
      if (parsed.data.source === "budget_match") {
        await updateBudgetLead(existing[0].id);
